...
public class DekalbWeatherV3_8_2 extends PApplet {
...
int stationlength =  PApplet.parseInt(stationlist.length);
...
if (SNOW.equals(sky[0]) == true){
  ...
  ///////////////========================
    ...
    particles.add (new Particle(snow, 10.0f) );
...
if (LIGHTRAIN.equals(skytwo) == true){
  ...
  ///////////////========================
    ...
    particles.add (new Particle(rain, 10.0f) );
  for (int i=particles.size()-1; i>=0; i--) {
    Particle p = (Particle)particles.get(i);
    p.update(); 
    if (p.position.y + p.diameter > height)
...
text(windpowertxt+"MPH",width-10,height/1.5f);
...
temptotalff= PApplet.parseInt(temptotalf[0]);
...
image(thermometer,50,height/1.75f,45,100);
rectMode(CENTER);
...
class Particle {
  ...
  Particle(PImage snowimg, Float winds) {
    ...
    PVector vel = new PVector(random(-20,20), random(0,10)*-1.0f);
    velocity = vel;
    snow2 = snowimg;
   winds= windtemp;
  }
...
cloud = tempcloud;
  ...
  public void display() {
   ...
   image(cloud,0,0);
   image(ground,0, height/1.4f,width,height/1.4f);
...
public void keyReleased() {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "DekalbWeatherV3_8_2" });
  }
}
